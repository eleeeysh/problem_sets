---
title: "Psych201a, Problem Set 2: Tidying Data"
author: "Andrew Lampinen, modified by Janna Wennberg, modified by Sihan Yang"
...
```{r}
suppressWarnings(library(foreign)) # for reading spss formatted data
suppressWarnings(library(tidyr))
suppressWarnings(library(dplyr))
suppressWarnings(library(stringr)) # useful for some string manipulation
...
```{r}
# comment out otherwise the rendered looks messy
# head(d)
```
...
```{r}
## your code here
uniq_vals <- unique(d$Game1Angry1) 
min_likert <- uniq_vals |> min(na.rm=TRUE)
max_likert <- uniq_vals |> max(na.rm=TRUE)
```

Highest number: `r min_likert`
Lowest number: `r max_likert`
...
filtered_d <- d |> filter(is.na(DoNotUse))
...
Cool, let's go back to the data:
```{r}
...
performance_summary_d = performance_diff_d |>
  ...
  summarise(MeanImprovementScore=mean(ImprovementScore), SEMImprovementScore=sem_func(ImprovementScore))
```
...
performance_summary_d |>
  ...
  ggplot(aes(x=ConfrontationalGame, y=MeanImprovementScore, fill=MusicCondition)) +
    ...
    geom_errorbar(
      position=pos, stat="identity",
      aes(ymin = MeanImprovementScore - SEMImprovementScore, 
        ymax = MeanImprovementScore + SEMImprovementScore), width=0.2) +
    scale_fill_brewer(palette="Set1")
```

(Bonus: also calculate the SEM in the summary data, and then add errorbars to the plot with `geom_errorbar`!)

Not quite as exact a replication of the effect as Fig. 1. This concurs with the [replication report](https://osf.io/7i2tf/), which says that the hypothesis 1 effect replicated, but hypothesis 2 did not. 
